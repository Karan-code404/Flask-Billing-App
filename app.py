...
with app.app_context():
    db.create_all()
    # Check if a user exists, if not, create a default admin user
    if not User.query.first():
        admin_user = User(username='admin', brand_name='Admin Brand')
        admin_user.set_password('password')
        db.session.add(admin_user)
        db.session.commit()
    # Check if items exist, if not, add sample items
...
def register():
    if request.method == 'POST':
        ...
        if existing_user:
            ...
            new_user = User(username=username, brand_name=brand_name)
...
def generate_pdf():
    ...
    # Get the logged-in user's brand name from the session
    username = session.get('username')
    user = User.query.filter_by(username=username).first()
    brand_name = user.brand_name if user and user.brand_name else "Your Brand Name"
    ...
    # Initialize PDF object and add bill content
    pdf = FPDF()
    pdf.add_page()
    pdf.set_font("Arial", size=16)

    # Print the brand name at the top of the bill
    pdf.cell(200, 10, text=brand_name, ln=True, align="C")
